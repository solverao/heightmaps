...
fn project(
    ...
    n_cells: f32,
    rot_sin: f32, rot_cos: f32,
    tilt_sin: f32, tilt_cos: f32,
    scale: f32,
    center: Pos2,
    elevation_scale: f32,
) -> Pos2 {
...
pub fn draw(
    ...
    let proj = |gx: f32, gy: f32, gz: f32| -> Pos2 {
        project(gx, gy, gz, n_cells, rot_sin, rot_cos, tilt_sin, tilt_cos,
                scale, center, elevation_scale)
    };
    ...
    let cell_n = n - 1;
    let ys_vec: Vec<usize> = if y_fwd { (0..cell_n).collect() } else { (0..cell_n).rev().collect() };
    let xs_vec: Vec<usize> = if x_fwd { (0..cell_n).collect() } else { (0..cell_n).rev().collect() };
    ...
    for &gy in &ys_vec {
        for &gx in &xs_vec {
            ...
            if ![p00, p10, p11, p01].iter().all(|p| clip.contains(*p)) { continue; }
            ...
            let diff   = (nx_n/n_len*light.0/l_len
                        ...
                        + nz_n/n_len*light.2/l_len).clamp(-1.0, 1.0);
